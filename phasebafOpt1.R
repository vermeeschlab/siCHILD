phasebafOpt1 <- function(Gtypes,dataraw,Family,Parent1,gamma,outPath,Parents){ 

	print("Computing phased BAF...")

	source(paste(siCHILD_DIR,"callpcfBAF.R",sep=""))
	source(paste(siCHILD_DIR,"phgrandparents.R",sep=""))
...
write.table(Parents,paste(outPath,Family,"debugParents.txt",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
	...
	Father <- Parents[,1]
	Mother <- Parents[,2]
write.table(dataraw,paste(outPath,Family,"debugdatarawBAF.txt",sep=""),col.names=T,row.names=F,quote=F,sep="\t")

	BAFs <- dataraw[,c(1:3,grep("^E",colnames(dataraw)),grep("Father.B",colnames(dataraw)),grep("Mother.B",colnames(dataraw)))]	
...
#	BAFstriX <- dataraw[dataraw$Chr == "X",grep("Mother.B",colnames(dataraw))]	
#}

	colnames(BAFs) <- gsub(".B.Allele.Freq","",colnames(BAFs))
	colnames(BAFs)[4:ncol(BAFs)] <- paste(colnames(BAFs)[4:ncol(BAFs)],"_",Family,sep="")
write.table(BAFs,paste(outPath,Family,"debugphasedBAF.txt",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
