htypingOpt5 <- function(Gtypes,dataPo,ParScore,Parent1,SibPattern,Family,outPath,Condition){

	print("*********************************************")
	print("****  Option5Htype: singleGP analysis..  ****")
	...
	#------------	Phasing chr X	-------------
	HapsX <- chrxhtypingOpt5(Gtypes,Parent1,ScSexes,Family)
...
print(paste0("~~~~~~",Seed))
	if (length(grep("Grandfather", Seed)==1) == 1) {	
		...
		PhasedPar[Gtypes[,Seed]=="BB" & Gtypes[,Parent1]=="AB" ] <- "BA"	
		PhasedPar[Gtypes[,Seed]=="AA" & Gtypes[,Parent1]=="AB" ] <- "AB"	

	} else if  (length(grep("Grandmother", Seed)==1) == 1) {	
		PhasedPar_RN <- c(rownames(Gtypes[Gtypes[,Seed]=="BB" & Gtypes[,Parent1]=="AB",]),rownames(Gtypes[Gtypes[,Seed]=="AA" & Gtypes[,Parent1]=="AB",]))
		PhasedPar[Gtypes[,Seed]=="BB" & Gtypes[,Parent1]=="AB" ] <- "AB"	
		PhasedPar[Gtypes[,Seed]=="AA" & Gtypes[,Parent1]=="AB" ] <- "BA"	
	}
	...
	for(sib in Sibs){
		...
		#-------------------------------------------------------------------------------------------------------
		## first allele is GF, second allele is GM => light color
		...
		## first allele is GF, second allele is GM =>  dark color
