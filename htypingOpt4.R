htypingOpt4 <- function(Gtypes,dataPo,ParScore,Parent1,SibPattern,Family,outPath,Condition){

	print("******************************************")
	print("****     Option4Htype analysis...     ****")
	...
	#------------	Phasing chr X	-------------
	HapsX <- chrxhtypingOpt4(Gtypes,Parent1,ScSexes,Family)
	...
	PhasedPar_RN <- rownames(Gtypes[(Gtypes[,Parent1]=="AA")|(Gtypes[,Parent1]=="BB"),])

	PhasedPar_RN <- c(PhasedPar_RN,rownames(Gtypes[Gtypes[,Seed]=="BB" & Gtypes[,Parent1]=="AB",]),rownames(Gtypes[Gtypes[,Seed]=="AA" & Gtypes[,Parent1]=="AB",]))
	PhasedPar[Gtypes[,Seed]=="BB" & Gtypes[,Parent1]=="AB" ] <- "BA"	# common & disease allele B
	PhasedPar[Gtypes[,Seed]=="AA" & Gtypes[,Parent1]=="AB" ] <- "AB"	# common & disease allele A

	PhasedPar_RN <- c(PhasedPar_RN,rownames(Gtypes[Gtypes[,Seed]=="AA" & Gtypes[,Parent1]=="AB",]),rownames(Gtypes[Gtypes[,Seed]=="BB" & Gtypes[,Parent1]=="AB",])  )
	PhasedPar[Gtypes[,Seed]=="AA" & Gtypes[,Parent1]=="AB" ] <- "AB"	# common & disease allele A
	PhasedPar[Gtypes[,Seed]=="BB" & Gtypes[,Parent1]=="AB" ] <- "BA"	# common & disease allele B
	...
	for(sib in Sibs){
		...
		#-------------------------------------------------------------------------------------------------------
		## first allele is disease allele, single cell inherited 1 allele => healthy? => light color
		...
		## first allele is disease allele, single cell inherited 2 allele => sick? => dark color
		...
		if(sib==Sibs[1]){
			ParHaps <- Hap1; ParHaps <- as.data.frame(ParHaps)
		}else{
