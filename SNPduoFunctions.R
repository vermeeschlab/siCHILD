...
#print(paste(cytobandInfo,"...............222222........"))
...
{
###write.table(cbind(ibs,position),"~/tmp/ibs.txt",col.names=F,row.names=F, quote=F)
#print(length(ibs))
...
#############################################
snpduo_single_chromosome = function( cytoband, genotypeData, chromosome, ind1, ind2, savename, pagewidth, pageheight, bychromosome = FALSE, cexScale = 0.20, comparison = "None", pchValue=20,doPostscript = TRUE, makeBED = TRUE, doPNG=FALSE, DPI=72, THUMBNAIL=FALSE )
{	
...
# <------------------------------------------------------
	...
	{	
		...
		if ( makeBED )
		{
			...
			ibs_blocks <- segment_ibs_blocks( ibs=ibsVector, position=physPos, chromosome=chromosome, Ind1=printInd1, Ind2=printInd2, File=paste(savename,"_", comparison,  ".bedsummary.txt", sep = "" ) )
		...
		ibs2_blocks <- ibs_blocks[ibs_blocks[,3] == "IBS2",]
		ibs_blocks_byChr <- cbind(rep(chromosome,nrow(ibs2_blocks)),ibs2_blocks)
		colnames(ibs_blocks_byChr)[1] <- "Chr"
		ibs2_blocks <- ibs_blocks_byChr
		ddply(ibs2_blocks, c('Chr', 'StartPosition'))
		write.table(ibs2_blocks,paste( savename,"_", comparison,  ".bedsummaryIBS2.txt", sep = "" ),sep="\t",col.names=T,row.names=F,quote=F)

		ibs1_blocks <- ibs_blocks[ibs_blocks[,3] == "IBS1",]
		ibs_blocks_byChr <- cbind(rep(chromosome,nrow(ibs1_blocks)),ibs1_blocks)
		colnames(ibs_blocks_byChr)[1] <- "Chr"
		ibs1_blocks <- ibs_blocks_byChr
		ddply(ibs1_blocks, c('Chr', 'StartPosition'))
		write.table(ibs1_blocks,paste( savename,"_", comparison,  ".bedsummaryIBS1.txt", sep = "" ),sep="\t",col.names=T,row.names=F,quote=F)



		ibs0_blocks <- ibs_blocks[ibs_blocks[,3] == "IBS0",]
		ibs_blocks_byChr <- cbind(rep(chromosome,nrow(ibs0_blocks)),ibs0_blocks)
		colnames(ibs_blocks_byChr)[1] <- "Chr"
		ibs0_blocks <- ibs_blocks_byChr
		ddply(ibs0_blocks, c('Chr', 'StartPosition'))
		write.table(ibs0_blocks,paste( savename,"_", comparison,  ".bedsummaryIBS0.txt", sep = "" ),sep="\t",col.names=T,row.names=F,quote=F)
	...
	} else if ( bychromosome )
	{
		...
		if ( makeBED )
		{
			ibs_blocks <- segment_ibs_blocks( ibs=ibsVector, position=physPos, chromosome=chromosome, Ind1=printInd1, Ind2=printInd2, File=paste( savename,"_", comparison,  ".bedsummary.txt", sep = "" ), AppendLogical=TRUE )
		...
		ibs2_blocks <- ibs_blocks_byChr
		ddply(ibs2_blocks, c('Chr', 'StartPosition'))
		write.table(ibs2_blocks,paste( savename, "chr", chromosome, "_", comparison,  ".bedsummaryIBS2.txt", sep = "" ),sep="\t",col.names=T,row.names=F,quote=F)
#print(savename)
		...
		ibs1_blocks <- ibs_blocks_byChr
		ddply(ibs1_blocks, c('Chr', 'StartPosition'))
		write.table(ibs1_blocks,paste( savename, "chr", chromosome, "_", comparison,  ".bedsummaryIBS1.txt", sep = "" ),sep="\t",col.names=T,row.names=F,quote=F)
		...
		ibs0_blocks <- ibs_blocks_byChr
		ddply(ibs0_blocks, c('Chr', 'StartPosition'))
		write.table(ibs0_blocks,paste( savename, "chr", chromosome, "_", comparison,  ".bedsummaryIBS0.txt", sep = "" ),sep="\t",col.names=T,row.names=F,quote=F)
...
#print(paste(doPostscript,doPNG,THUMBNAIL,"here-----"))				
...
#print(paste("//////-----",chromosome, cytoband))
...
#------------------#
			...
			points( x = ibs2_blocks[,2], y = rep(mean(ibs.jittered[complete.cases(ibs.jittered)]),dim(ibs2_blocks)[1]), cex = 1.05,pch = 24 , col="green")
 			points( x = ibs2_blocks[,3], y = rep(mean(ibs.jittered[complete.cases(ibs.jittered)]),dim(ibs2_blocks)[1]), cex = 1.05,pch = 25 , col="red")

			points( x = ibs0_blocks[,2], y = rep(mean(ibs.jittered[complete.cases(ibs.jittered)]),dim(ibs0_blocks)[1]), cex = 1.05,pch = 3 , col="green")
 			points( x = ibs0_blocks[,3], y = rep(mean(ibs.jittered[complete.cases(ibs.jittered)]),dim(ibs0_blocks)[1]), cex = 1.05,pch = 4 , col="red")
...
# a single image.                      #
########################################
whole_genome_plot = function( cytoband, genotypeData, ind1, ind2, savename, pagewidth, pageheight, cexScale=0.20, comparison="None", pchValue=20, doPostscript=TRUE, makeBED = TRUE, doPNG=FALSE, DPI=72,chr.offset=NULL )
{
...
###write.table(genotypeData,"tmp1.txt",col.names=F, row.names=F, sep="\t", quote=F)

	gen1 = genotypeData[,ind1] 
	gen2 = genotypeData[,ind2] 
...
# <===============================
	...
	if ( makeBED )
	{
		chromList = sort(as.numeric(unique( genotypeData[,chr] )))
		ibs2_blocks_ALL <- NULL
		for ( chromIndex in 1:length( chromList ) )
		{
			...
			chrom = chromList[ chromIndex ]
			
			ibs_blocks <- segment_ibs_blocks( ibs=ibsTmp, position=positionTmp, chromosome=chromList[ chromIndex ], Ind1=printInd1, Ind2=printInd2, File=paste( savename,"_", comparison,  ".bedsummary.txt", sep = "" ), AppendLogical=TRUE )

			ibs2_blocks <- ibs_blocks[ibs_blocks[,3] == "IBS2",]

			ibs2_blocks_byChr <- cbind(rep(chrom,nrow(ibs2_blocks)),ibs2_blocks)
			colnames(ibs2_blocks_byChr)[1] <- "Chr"

			ibs2_blocks_ALL <- rbind(ibs2_blocks_ALL,ibs2_blocks_byChr)
		}
		ibs2_blocks <- ibs2_blocks_ALL
		ddply(ibs2_blocks, c('Chr', 'StartPosition'))
		##write.table(ibs2_blocks,paste( savename,"_", comparison,  ".000000000bedsummaryIBS2.txt", sep = "" ),sep="\t",col.names=T,row.names=F,quote=F)
...
###########################################
genome_by_chromosome = function ( cytoband,genotypeData, ind1, ind2, savename, pswidth, psheight, comparison = "None", doPostscript = TRUE, chromlist = NULL, makeBED = TRUE, doPNG=FALSE, DPI=72 )
{
	printInd1 = gsub( "\\.", " ", names( genotypeData )[ ind1 ] )
	printInd2 = gsub( "\\.", " ", names( genotypeData )[ ind2 ] )
	...
	{
		chrom = chromlist[i]
		snpduo_single_chromosome( cytoband, genotypeData,chrom, ind1, ind2, savename, pswidth, psheight, bychromosome = TRUE, comparison = comparison, doPostscript = doPostscript, makeBED = makeBED, doPNG=doPNG, DPI=DPI, THUMBNAIL=TRUE )
	}
}
