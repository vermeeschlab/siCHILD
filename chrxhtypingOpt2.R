chrxhtypingOpt2 <- function(Gtypes,ParScore,SibPattern){
	...
	RefSib <- Gtypes[Gtypes$Chr=="X",grep(SeedCol,colnames(Gtypes))]


	ScSexes <- matrix(NA,length(ParScore),2)
	rownames(ScSexes)<-names(ParScore)

#	if ((any(grepl("Sibling", names(ParScore)))) & (Seed == "Sibling")) {
#		names(ParScore) <- gsub("Sibling","Affected",names(ParScore))
#	}
...
print(sc)
	...
	if (ScSexes[grep(SeedCol,ScSexes[,1]),2] =="female") {
	...
	Bls<- Gtypes[Gtypes$Chr=="X",ScSexes[unique(c(grep(SibPattern,ScSexes[,1]),grep(SeedCol,ScSexes[,1]))),1]]#Blastomeres
	#}else{
	...
	#if(sum(grep("REF",Family))!=1){
	colnames(matHaps)<-paste(colnames(Gtypes)[unique(c(grep(SibPattern,colnames(Gtypes)),grep(SeedCol,colnames(Gtypes))))],"_Mat",sep="")
	colnames(patHaps)<-paste(colnames(Gtypes)[unique(c(grep(SibPattern,colnames(Gtypes)),grep(SeedCol,colnames(Gtypes))))],"_Pat",sep="")
	#}else{
