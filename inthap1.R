...
for(h in colnames(dataHap)[-c(1:3)]){
	...
	for(chr in Chroms){
		...
		HapIntGenome<-c(HapIntGenome,HapInt)
...
if(vb>1){
	...
	HapBlocksIntGenome <- c(HapBlocksIntGenome,HapBlocksChrIntFinal)


}#end chr loop

HapIntFamily <- cbind(HapIntFamily,HapBlocksIntGenome)
