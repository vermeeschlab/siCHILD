phasebafOpt4 <- function(Gtypes,dataraw,Family,Parent1,gamma,outPath,Parents){ 
...
write.table(Parents,paste(outPath,Family,"debugParents.txt",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
	...
	BAFs <- BAFs[!(Gtypes[,Seed2]=="NC"),]
	Father <- Father[!(Gtypes[,Seed2]=="NC")]
	Mother <- Mother[!(Gtypes[,Seed2]=="NC")]
