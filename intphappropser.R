intphappropser <- function(dataPath,Family,ParScore,SibPattern){
...
Blasts <- colnames(dataHap)[c(grep("Affect",colnames(dataHap)),grep(SibPattern,colnames(dataHap)))]
...
#print(i)
					if(Frac[i]=="NaN" | is.na(Frac[i])){Frac[i] <- 0}#when no informative SNP is present for the smoothed haplotype block
			...
			print(paste("The origin of chr.",chr, "of", Indiv,"could not be determined"))
		}
