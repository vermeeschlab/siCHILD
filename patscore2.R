...
#print(ind)
		...
		for(chr in Chroms) { 
			...
			if(is.na(Pat) || (Pat==0)){Pat=0.0001} ; if(is.na(Mat) || (Mat==0)){Mat=0.0001}
			PatMat <- Pat/(Mat+Pat)
			MatPat <- Mat/(Mat+Pat)
...
#			ind2 <- gsub("E0Bl0_","",ind2)
	...
	for(ind in names(SamplesPo)){
		...
		## original cutoff = 0.90, to reproduce plot for PGD427_C1 E05 Chr16 (which is a trisomy case), the value is increased to be 0.95 ##
		Sample[Sample[,"CallRate"]>MinCallRate & ((Sample[,"Pat"]< (AvgMDA+SdMDA)& Sample[,"Mat"]< (AvgMDA+SdMDA)) | (Sample[,"PatMat"]<0.95 & Sample[,"MatPat"]<0.95)),"Par"]<- 12	# female
		Sample[Sample[,"Pat"]> (AvgMDA+SdMDA) & Sample[,"CallRate"]>MinCallRate & Sample[,"PatMat"]>=0.95 ,"Par"] <- 11	# female, without maternal X
		Sample[Sample[,"Mat"]> (AvgMDA+SdMDA) & Sample[,"CallRate"]>MinCallRate & Sample[,"MatPat"]>=0.95 ,"Par"] <- 22	# male
		Sample[Sample[,"CallRate"]<MinCallRate & (Sample[,"PatMat"]<0.5 & Sample[,"MatPat"]<0.5) ,"Par"] <- 00	# chrX nullisomy
