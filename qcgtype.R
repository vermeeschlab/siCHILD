...
Sibs <- c(grep(SibPattern,colnames(Gtypes)),grep("Sibling",colnames(Gtypes))) 
ColNames <- colnames(Gtypes)
#ColNames<-gsub("Sibling","",colnames(Gtypes))
...
for(sib in Sibs){
	...
	MendIncRateChrsInd <- rbind(MendIncRateChrsInd,MendIncRateChrs)
